...
#include <unistd.h>  
/* Prototype */
//static void print_stream_stat(pjmedia_stream *stream, const pjmedia_codec_param *codec_param);
...
int main_rtp_send_rece( struct info_rtp *info_rtp_temp)
{
    ...
    pj_uint16_t local_port =4001;
	pj_str_t s;
	pj_str_t* IP_Addr =NULL;
...
//	OPT_SRTP_RX_KEY	= 'y',
	...
	/*Set the codec id*/
	if(NULL !=info_rtp_temp->codec_id)
		codec_id = info_rtp_temp->codec_id;

	/*Set the local port*/	
	if(info_rtp_temp->local_port > 0)
		local_port =info_rtp_temp->local_port;
	
	/*Use the IPv4 */
	remote_addr.sin_family=pj_AF_INET(); 	 
	/*Set the IPv4 PORT */
	if(info_rtp_temp->remote_port > 0)
    	remote_addr.sin_port=pj_htons(info_rtp_temp->remote_port);
	else 
		remote_addr.sin_port=pj_htons(4003);
	
	/*Set the IPv4 ADDR */
	if(NULL !=info_rtp_temp->remote_addr)
    	IP_Addr=pj_cstr(&s,info_rtp_temp->remote_addr);
	else 
		IP_Addr=pj_cstr(&s,"192.168.1.110");
	
    remote_addr.sin_addr=pj_inet_addr(IP_Addr);
